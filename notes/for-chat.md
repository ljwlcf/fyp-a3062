...
- **Does the graph-quality finding weaken the Phase 2 primary candidate?** Measured on the
  built graphs (2026-09-20): ~80% of django's `invokes` edges and ~75% of sphinx's are
  one-call-name-to-many-targets, i.e. name matches the builder never resolved; a single
  call to `get()` wires the caller to 614 different django methods. "Graph-grounded debate"
  ...
  finishes, but it changes what Phase 1 needs to measure. (notes/results.md 2026-09-20)
